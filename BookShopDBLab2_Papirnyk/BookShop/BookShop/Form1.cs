...
namespace BookShop
{
    public partial class Form1 : Form
    {
        ...
        {
            string connectionString = @"Data Source=DESKTOP-O35RURI;Initial Catalog=InternetShopDB;Integrated Security=True";
            sqlConnection = new SqlConnection(connectionString);
            await sqlConnection.OpenAsync();
            SelectBox.DropDownStyle = ComboBoxStyle.DropDownList;
            ComboBoxHardQuery.DropDownStyle = ComboBoxStyle.DropDownList;
        ...
        private async void SelectButton_Click(object sender, EventArgs e)
        {
            ...
            if (SelectBox.SelectedIndex == 1)
            {
                ...
                try
                {
                    sqlReader = await command.ExecuteReaderAsync();
                    listBox1.Items.Clear();
                    listBox1.Items.Add(Convert.ToString("ID продукта" + "  |  " + "Автор" + "  |  " + "Описание"));
                    listBox1.Items.Add(Convert.ToString("---------------------------------------------------------------------------------"));
                    while (await sqlReader.ReadAsync())
                    {
                        counter++;
                        listBox1.Items.Add(Convert.ToString(sqlReader["ID"]) + "  |  " + Convert.ToString(sqlReader["Author"]) + "  |  " + Convert.ToString(sqlReader["Description"]));
                    }
                    if (counter > 0)
                    {
                        MessageBox.Show(string.Format("В таблице было найдено и выведено на экран {0} строк(и)", counter), "Отчёт :)", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private async void buttonProdInsert_Click(object sender, EventArgs e)
        {
        ...
        private async void buttonProdDetInsert_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(textBoxProdDetIDInsert.Text) && !string.IsNullOrWhiteSpace(textBoxProdDetIDInsert.Text) &&
                   !string.IsNullOrEmpty(textBoxProdDetAuthorInsert.Text) && !string.IsNullOrWhiteSpace(textBoxProdDetAuthorInsert.Text) &&
                   !string.IsNullOrEmpty(textBoxProdDetDescriptionInsert.Text) && !string.IsNullOrWhiteSpace(textBoxProdDetDescriptionInsert.Text))
                {

                    SqlCommand command = new SqlCommand("INSERT INTO [ProductDetails] (ID, Author, Description)VALUES(@ID, @Author, @Description)", sqlConnection);

                    command.Parameters.AddWithValue("ID", textBoxProdDetIDInsert.Text);
                    command.Parameters.AddWithValue("Author", textBoxProdDetAuthorInsert.Text);
                ...
                textBoxProdDetAuthorInsert.Clear();
        ...
        private async void buttonEmployeesInsert_Click_1(object sender, EventArgs e)
        {
        ...
        private async void buttonEmployeesInfoInsert_Click_1(object sender, EventArgs e)
        {
        ...
        private async void buttonOrderDelete_Click(object sender, EventArgs e)
        {
            ...
            if (warningOrderDelete.Visible)
                warningOrderDelete.Visible = false;
            try
            {
                if (!string.IsNullOrEmpty(textBoxOrderIDDelete.Text) && !string.IsNullOrWhiteSpace(textBoxOrderIDDelete.Text))
                {

                    SqlCommand command = new SqlCommand("DELETE FROM [Orders] WHERE [ID]=@ID", sqlConnection);

                    command.Parameters.AddWithValue("ID", textBoxOrderIDDelete.Text);

                    await command.ExecuteNonQueryAsync();
                }
                else
                {
                    warningOrderDelete.Visible = true;
                    warningOrderDelete.Text = "Id должны быть заполнены!";
                }
                textBoxOrderIDDelete.Clear();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message.ToString(), ex.Source.ToString(), MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private async void buttonProdDelete_Click(object sender, EventArgs e)
        {
            if (warningProdDelete.Visible)
                warningProdDelete.Visible = false;
            try
            {
                if (!string.IsNullOrEmpty(textBoxProdIDDelete.Text) && !string.IsNullOrWhiteSpace(textBoxProdIDDelete.Text))
                {

                    SqlCommand command = new SqlCommand("DELETE FROM [Products] WHERE [ID]=@ID", sqlConnection);

                    command.Parameters.AddWithValue("ID", textBoxProdIDDelete.Text);

                    await command.ExecuteNonQueryAsync();
                }
                else
                {
                    warningProdDelete.Visible = true;
                    warningProdDelete.Text = "Id должны быть заполнены!";
                }
                textBoxProdIDDelete.Clear();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message.ToString(), ex.Source.ToString(), MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private async void buttonProdDetDelete_Click(object sender, EventArgs e)
        {
            if (warningProdDetailsDelete.Visible)
                warningProdDetailsDelete.Visible = false;
            try
            {
                if (!string.IsNullOrEmpty(textBoxProdDetailsIDDelete.Text) && !string.IsNullOrWhiteSpace(textBoxProdDetailsIDDelete.Text))
                {

                    SqlCommand command = new SqlCommand("DELETE FROM [ProductDetails] WHERE [ID]=@ID", sqlConnection);

                    command.Parameters.AddWithValue("ID", textBoxProdDetailsIDDelete.Text);

                    await command.ExecuteNonQueryAsync();
                }
                else
                {
                    warningProdDetailsDelete.Visible = true;
                    warningProdDetailsDelete.Text = "Id должны быть заполнены!";
                }
                textBoxProdDetailsIDDelete.Clear();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message.ToString(), ex.Source.ToString(), MessageBoxButtons.OK, MessageBoxIcon.Error);
       ...
       //HARD QUERY
        ...
        private async void buttonHardQuery_Click(object sender, EventArgs e)
        {

            int counter = 0;
            if (ComboBoxHardQuery.SelectedIndex == 0)
            {
                SqlDataReader sqlReader = null;
                ...
                try
                {
                    pictureBox1.Load("innerjoin.png");
                    sqlReader = await command.ExecuteReaderAsync();
                    listBox2.Items.Clear();
                    listBox2.Items.Add(Convert.ToString("Inner join - выводится то, что есть в обеих таблицах, фактически - их пересечение"));
                    listBox2.Items.Add(Convert.ToString("---------------------------------------------------------------------------------"));
                    listBox2.Items.Add(Convert.ToString("Наименование" + "  |  " + "Автор" + "  |  " + "Описание"));
                    listBox2.Items.Add(Convert.ToString("---------------------------------------------------------------------------------"));
                    while (await sqlReader.ReadAsync())
                    {
                        counter++;
                        listBox2.Items.Add(Convert.ToString(sqlReader["Name"]) + "  |  " + Convert.ToString(sqlReader["Author"]) + "  |  " + Convert.ToString(sqlReader["Description"]));
                    }
                    if (counter > 0)
                    {
            ...
            if (ComboBoxHardQuery.SelectedIndex == 1)
            {
                SqlDataReader sqlReader = null;
                ...
                try
                {
                    pictureBox1.Load("fulljoin.png");
                    sqlReader = await command.ExecuteReaderAsync();
                    listBox2.Items.Clear();
                    listBox2.Items.Add(Convert.ToString("Full join - выводится вся информация из двух таблиц, даже если на складе нет таких товаров"));
                    listBox2.Items.Add(Convert.ToString("---------------------------------------------------------------------------------"));
            ...
            if (ComboBoxHardQuery.SelectedIndex == 2)
            {
                SqlDataReader sqlReader = null;
                ...
                try
                {
                    pictureBox1.Load("rightjoin.png");
                    sqlReader = await command.ExecuteReaderAsync();
                    listBox2.Items.Clear();
                    listBox2.Items.Add(Convert.ToString("Right join - выводится вся информация из правой таблицы, и связанные с ней элементы из левой"));
                    listBox2.Items.Add(Convert.ToString("---------------------------------------------------------------------------------"));
            ...
            if (ComboBoxHardQuery.SelectedIndex == 3)
            {
                SqlDataReader sqlReader = null;
                ...
                try
                {
                    pictureBox1.Load("leftjoin.png");
                    sqlReader = await command.ExecuteReaderAsync();
                    listBox2.Items.Clear();
                    listBox2.Items.Add(Convert.ToString("Left join - выводится вся информация из левой таблицы, и связанные с ней элементы из левой"));
                    listBox2.Items.Add(Convert.ToString("---------------------------------------------------------------------------------"));
                    listBox2.Items.Add(Convert.ToString("Наименование" + "  |  " + "Автор" + "  |  " + "Описание"));
                    listBox2.Items.Add(Convert.ToString("---------------------------------------------------------------------------------"));
                    while (await sqlReader.ReadAsync())
       ...
       //UPDATE
        ...
        private async void buttonProductDetailsUpdate_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(textBoxProductDetailsIDUpdate.Text) && !string.IsNullOrWhiteSpace(textBoxProductDetailsIDUpdate.Text) &&
                    !string.IsNullOrEmpty(textBoxProductDetailsAuthorUpdate.Text) && !string.IsNullOrWhiteSpace(textBoxProductDetailsAuthorUpdate.Text) &&
                    !string.IsNullOrEmpty(textBoxProductDetailsDescriptionUpdate.Text) && !string.IsNullOrWhiteSpace(textBoxProductDetailsDescriptionUpdate.Text))
                {
                    ...
                    SqlCommand command = new SqlCommand("UPDATE [ProductDetails] SET [Author]=@Author, [Description]=@Description WHERE [ID]=@ID", sqlConnection);

                    command.Parameters.AddWithValue("ID", textBoxProductDetailsIDUpdate.Text);
                    command.Parameters.AddWithValue("Author", textBoxProductDetailsAuthorUpdate.Text);
                ...
                textBoxProductDetailsAuthorUpdate.Clear();
        ...
        private async void buttonFilter_Click_1(object sender, EventArgs e)
        {
            try
            {
                if (warningFilter.Visible)
                    warningFilter.Visible = false;

                if (!string.IsNullOrEmpty(textBoxFilter1.Text) && !string.IsNullOrWhiteSpace(textBoxFilter1.Text))
                {
                    SqlDataReader sqlReader = null;
                    SqlCommand command = new SqlCommand("SELECT * FROM [OrderDetails] WHERE OrderDetails.Price > @Price", sqlConnection);

                    command.Parameters.AddWithValue("Price", textBoxFilter1.Text);
                    ...
                    try
                    {
                        sqlReader = await command.ExecuteReaderAsync();
                        listBox2.Items.Clear();
                        listBox2.Items.Add(Convert.ToString("ID заказа" + "  |  " + "Приоритет" + "  |  " + "ID продукта" + "  |  " + "Количество" + "  |  " + "Цена" + "  |  " + "Общая цена"));
                        listBox2.Items.Add(Convert.ToString("---------------------------------------------------------------------------------"));
                        while (await sqlReader.ReadAsync())
                        {
                            counter++;
                            listBox2.Items.Add(Convert.ToString(sqlReader["OrderID"]) + "  |  " + Convert.ToString(sqlReader["LineItem"]) + "  |  " + Convert.ToString(sqlReader["ProductID"] + "  |  " + Convert.ToString(sqlReader["Qty"]) + "  |  " + Convert.ToString(sqlReader["Price"]) + "  |  " + Convert.ToString(sqlReader["TotalPrice"])));
                        }
                        if (counter > 0)
                        {
                            MessageBox.Show(string.Format("В таблице было найдено и выведено на экран {0} строк(и)", counter), "Отчёт :)", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                {
                    warningFilter.Visible = true;
                    warningFilter.Text = "Все поля должны быть заполнены!";
                }
